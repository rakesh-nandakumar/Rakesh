...
const STORAGE_BASE_URL = supabaseUrl
  ? `${supabaseUrl}/storage/v1/object/public/portfolio`
  : "";
...
export function getStorageUrl(bucketOrPath, path) {
  ...
  if (
    bucketOrPath &&
    (bucketOrPath.startsWith("http://") || bucketOrPath.startsWith("https://"))
  ) {
    return bucketOrPath;
  }
  ...
  if (typeof data === "object") {
    ...
    for (const field of imageFields) {
      if (transformed[field]) {
        transformed[field] = getStorageUrl(
