...
import { supabaseAdmin, getStorageUrl, STORAGE_BUCKETS, getSupabaseAdmin } from './supabase';
...
const getDefaultImageUrl = (filename) => SUPABASE_STORAGE_URL ? `${SUPABASE_STORAGE_URL}/${filename}` : `/${filename}`;
...
function getDefaultAbout() {
  return {
    ...
    shortBio: 'Full Stack Developer passionate about creating innovative solutions.',
    longBio: '',
    contact: {}
  };
}
...
function transformProfileData(profile, titles, contacts, contactTypes) {
  ...
  return {
    ...
    contactImage: profile.contact_image ? getImageUrl(profile.contact_image, null) : null,
...
export const getAbout = cache(async () => {
  return getCachedData('about', async () => {
    try {
      ...
      return {
        name: "Rakesh Nandakumar",
        title: ["Software Engineer", "Technical Consultant", "Full Stack Developer"],
        profileImage: getDefaultImageUrl('profileImg.png'),
        heroImage: getDefaultImageUrl('hero.jpg'),
        cvLink: getDefaultImageUrl('cv.pdf'),
...
export const getBlogs = cache(async () => {
  return getCachedData('blogs', async () => {
    try {
      ...
      return blogs.map(blog => transformBlogData(blog, tagsByBlogId.get(blog.id) || []));
    } catch (error) {
 ...
 */
function transformPortfolioData(portfolio, technologies = [], keyFeatures = []) {
 ...
 */
function transformTimelineData(timeline, technologies = [], links = [], categoryName) {
  return {
    id: timeline.id,
...
function transformHeaderData(header, navigations = [], ctas = []) {
  // Sort by sort_order
  const sortedNav = [...navigations].sort((a, b) => (a.sort_order || 0) - (b.sort_order || 0));
  const sortedCtas = [...ctas].sort((a, b) => (a.sort_order || 0) - (b.sort_order || 0));
...
export const getHeader = cache(async () => {
  return getCachedData('header', async () => {
    try {
      ...
      return transformHeaderData(headers[0], navResult.data || [], ctaResult.data || []);
    } catch (error) {
      ...
      return {
        ...
        ctaButton: { label: "Download CV", href: getDefaultImageUrl('cv.pdf'), target: "_blank" }
      };
    }
  });
});
